...
public class ActorDao {
  ...
  // IS THIS CORRECT!??
  // I want to return an array of actor objects for the film object to sort through...

  public List<Map<String, Object>> getActorsForFilm(Integer filmId) {
    return jdbcTemplate.queryForList(String.format("SELECT a. *, (SELECT SUM(r.RATING_VALUE)  FROM ACTOR_RATING r WHERE r.GENRE_ID=f.GENRE_ID AND a.id=r.ACTOR_ID)  as rating FROM film_actor fa LEFT JOIN actor a on fa.ACTOR_ID=a.id LEFT JOIN film f on f.id=fa.FILM_ID WHERE fa.FILM_ID = %s AND a.DELETED=0;", filmId), new ActorRowMapper());
  }
